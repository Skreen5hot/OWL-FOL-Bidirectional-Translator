...
### 10. OFBT meta-vocabulary encoding choice — implicit-typing of class / object-property IRIs [ACCEPTED with Q-Frank-2 boundary-statement refinement, promoted at Q-Frank-Step9-A Ask 3 ruling 2026-05-10]

Architect-ratified at the BFO/CLIF parity routing cycle 2026-05-03 ("yes the elision is sound"); formalized at Phase 1 exit Step 9.4 doc pass; **promoted to Accepted with Q-Frank-2 boundary-statement refinement at the Q-Frank-Step9-A corrective ruling 2026-05-10** per Frank's §4.3 critique that ADR-007 §10 had been referenced as "architect-ratified" across three demo cycles without an explicit Accepted status. The promotion is ratification confirmation, not re-litigation.

**Boundary statement (per Q-Frank-2 ratification refinement 2026-05-07).** OFBT's elision is sound w.r.t. OWL semantics under the **implicit-typing assumption**: every IRI used in a class position is treated as if `(Class IRI)` were asserted; every IRI used in an object-property position is treated as if `(OWLObjectProperty IRI)` were asserted. The assumption is **bounded to OWL 2 DL inputs**: the lifter's `rejectPuntedConstructs` (per spec §13.1, exercised by `canary_punned_construct_rejection.fixture.js`) rejects inputs where the same IRI appears in conflicting positional roles — which is exactly the case where implicit-typing would corrupt soundness. Inside OWL 2 DL (no punning, syntactic role determines semantic role), implicit-typing is a sound encoding choice; outside OWL 2 DL (e.g., RDF schemas with type-promiscuous IRIs), the boundary is breached and OFBT's lifter refuses to lift. The soundness claim is therefore: *for OWL 2 DL inputs, OFBT's lifted FOL is sound w.r.t. OWL Direct Semantics modulo elision of recoverable meta-typing predicates.*
...
**Promotion audit trail (2026-05-10).** Q-Frank-Step9-A Ask 3 ruled the §10 promotion to Accepted at the Phase 3 close corrective commit. Frank's Phase 2 critique (Q-Frank-2 ratification cycle 2026-05-07) had ratified the §10 shape with the boundary-statement refinement above; Frank's Phase 3 critique (Q-Frank-Step9-A 2026-05-10 §4.3) flagged that despite Q-Frank-2 ratification, §10 had been referenced as "architect-ratified" across the Phase 1 + Phase 2 + Phase 3 demos without an explicit Accepted-status ADR section. Three demo cycles is enough — the promotion lands at Phase 3 close. Per Q-Frank-Step9-A Ask 3 directive, no further architect routing on §10 substance is required; this commit confirms the final landed text matches the Q-Frank-2 ratified shape.
...
| `FOLNegation` (¬P) | **EvaluableQuery**: rejected per API §7.5. **Implication body**: Prolog `\+ p(X)` (negation-as-failure). Spec §6.3 default-OWA: `\+ p` succeeding maps to `'undetermined'` (no proof) NOT `'false'`, UNLESS predicate `p` is in `closedPredicates`. **If in `closedPredicates`:** `\+ p` → `'false'` with reason **`inconsistent`** (REUSED existing reason code per Q-3-Step3-B refinement 1, 2026-05-09 — the closed-predicate NAF case is structurally a refutation under closed-world semantics, matching the existing `inconsistent` reason; no 18th `closed_world_negation` reason code introduced). **If predicate NOT in `closedPredicates`:** `'undetermined'` with reason **`open_world_undetermined`** (REUSED existing reason code per Q-3-Step4-A refinement, 2026-05-09 — corrects an editorial reference error in the Pass 2b ratified text where `naf_residue` was written instead of the canonical reason-code label; `open_world_undetermined` is the canonical reason code per `src/kernel/reason-codes.ts:31` for the OWA-stance "neither query nor its negation provable" semantic, exactly the spec §6.3 default-OWA framing this row describes; the `naf_residue` lossType per API §6.4.1 is the projector-side audit-artifact label and is structurally distinct from any evaluator-side reason code). | **Step 4 forward-track** (closedPredicates implementation) | `inconsistent` (closed-predicate); `open_world_undetermined` (open-predicate default OWA) |
...
**Reason enum stability:** The translation rules introduce no new reason codes. `inconsistent`, `open_world_undetermined`, and `coherence_indeterminate` all exist in the v0.1.7-frozen reason enum (16 members at v0.1.7 freeze; `no_strategy_applies` adds at Step 8 per Q-3-C 2026-05-08 ratification → 17 members at Phase 3 close). The closed-predicate NAF case reuses `inconsistent` per Q-3-Step3-B refinement 1 (2026-05-09); the open-predicate NAF case reuses `open_world_undetermined` per Q-3-Step4-A refinement (2026-05-09) — both are deliberate architectural choices to keep the reason enum stable. The `naf_residue` lossType per API §6.4.1 is the projector-side audit-artifact label and is structurally distinct from any evaluator-side reason code — the two surfaces correlate semantically (both name NAF-failure-to-prove cases) but use different vocabularies because they record different events (projector audit at lift time vs evaluator output at query time).
