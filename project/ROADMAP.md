...
| 9 | 100-run determinism harness per API §6.1.1; `verifiedStatus: 'Draft'` → `'Verified'` promotion across all 14 fixtures (incl. BFO); SME B3 (`p1_complement_of` for ObjectComplementOf coverage); SME O1 audit-trail block-b doc pass; risk retrospective; Phase 1 exit summary; the architect's two forward-tracked items (path-fencing protocol formalization as `arc/AUTHORING_DISCIPLINE.md` section 0; four-contract consistency check folded into checklist) | ⏳ Pending | (drives Phase 1 Exit Review) |

### Deliverables Checklist
Per-deliverable status reflects which Steps have landed. `~` indicates partially complete; `✅` complete; `⏳` pending.

- [~] `owlToFol()` per API spec §6.1 with structured OWL input handling per API spec §3 (Steps 1-8 complete; only Step 9 outstanding — Phase 1 exit deliverables: 100-run determinism harness, verifiedStatus promotions, SME B3 fixture, eight banked architect items, risk retrospective, Phase 1 exit summary)
