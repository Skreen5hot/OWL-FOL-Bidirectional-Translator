...
**Current Phase:** Phase 1 — Built-In OWL Lifter (IN PROGRESS, Steps 1-8 of 9 complete; CI green locally on Step 8 close pending push; CI green on `7e7e286` Step 7 close run `25315820872` and on `e1a4973` SME demo run `25318866263`). ADR-007 ratified Accepted at Step 5 close per architect Ruling 1; §7 (cardinality witness convention) resolved at Step 7 close; §8 (RDFC-1.0 b-node Skolem prefix) resolved at Step 6 close; spec §5.6.5 datatype canonicalization + spec §5.9.1 structural annotation declaration consistency machinery skeleton implemented at Step 8 close. **Phase 1 corpus Ring 1 closed at Step 7** — 14 fixtures + 10 inline regressions (B1×2, b-node IRI, Step 8 datatype canon ×6, Step 8 structural annotation ×2) all active and passing. Phase 0 EXITED 2026-05-02.
...
| 9 | Phase 1 exit deliverables, sub-stepped: **9.1** SME B3 fixture (`p1_complement_of` for ObjectComplementOf coverage) ✅ commit `f21cc24`; **9.2** [VERIFY] → Verified resolution on all 8 BFO/CLIF citations ✅ commit `209a531` (CI run `25330758341`); **9.3** 100-run determinism harness per API §6.1.1 (`tests/determinism-100-run.test.ts`; 15 fixtures × 100 runs = 2,100 lift invocations; 233ms; covers single-input, multi-input, and multi-throwing-case fixture shapes) ✅ this commit; **9.4** ADR-007 §10 + AUTHORING_DISCIPLINE doc pass (12 banked items + 2 opportunistic) ⏳ pending; **9.5** `verifiedStatus: 'Draft'` → `'Verified'` promotion across all 15 fixtures ⏳ pending; **9.6** Phase 1 exit summary at `project/reviews/phase-1-exit.md` + risk retrospective ⏳ pending | 🟡 In progress (9.1–9.3 ✅; 9.4–9.6 ⏳) | (drives Phase 1 Exit Review) |
...
### Phase 1 Exit Review
- [ ] All 15 corpus fixtures pass against running `owlToFol` (Ring 1) — 25/25 lifter tests passing as of Step 9.1
- [x] 100-run determinism on each fixture per API §6.1.1 — `tests/determinism-100-run.test.ts` (Step 9.3): 15/15 fixtures × 100 runs = 2,100 lift invocations, all canonicalized outputs byte-identical
