...
| 4b | Cardinality n-tuple matcher per ADR-012 (architect-ratified Option β at Step 4 spec-binding cycle 2026-05-07; captured in committed form at `a469371`): MinCardinality / MaxCardinality / ExactCardinality detected from the lifter's `∃y₁..yₙ` (with pairwise distinctness) / `∀y₁..y_{n+1}` (with pairwise-equality disjunction) / Min∧Max-conjunction shapes per ADR-007 §7. QCR `onClass` (NamedClass fillers) supported; recursive class-expression `onClass` deferred. Round-trip byte-clean against `p1_restrictions_cardinality`; manifest entry amended `regime: "reversible" → "equivalent"` + `expectedLossSignatureReasons: []` per ADR-012 banked principle 2. | ✅ Complete (this commit) |
...
- [x] **Phase 4 entry forward-tracks (post-Q-Frank-Step9-A corrective ruling 2026-05-10):** `cycle_equivalent_classes` Class-3 fixture re-binding; `nc_bfo_continuant_occurrent` + `nc_sdc_gdc` Phase-4-deferred fixtures; parallel-registry reconciliation (DECISIONS.md ADR-NN ↔ spec §13 ADR-NN); substantive-scope-weighting methodology refinement (now with full Phase 3 data: ~3 projected vs 5 actual); at-risk-tag-conservatism observation; LossType trigger-matcher subsystem cleanup; Realization regularity-check upgrade; **Case C — Lossy round-trip — as Phase 4 EXIT DELIVERABLE per Q-Frank-Step9-A Ask 4 (three-cycle drift on Frank's stakeholder flag triggered explicit phase-deliverable commitment, not forward-track)**; **Phase 2 reactivation surface as Phase 4 demo opening per Q-Frank-Step9-A Ask 7 SME election**; **purpose-built Layer A consistency-check parity fixture question per Q-Frank-Step9-A Ask 8**; **banking-correction discipline retro analysis per Q-Frank-Step9-A meta-banking**. **WITHDRAWN forward-tracks per Q-Frank-Step9-A corrective ruling:** ~~`nc_horn_incomplete_disjunctive` count-semantics corpus-discriminator-scope question~~ (resolved on the merits at Ask 2 via fixture amendment `expectedUnverifiedAxiomsMinCount: 2 → 1`); ~~disposition-split discipline as Phase-4-entry methodology candidate~~ (withdrawn at Ask 5; banked principle: disciplines invented in response to specific findings do not get banked as architectural-commitment-tier methodology in the same cycle).
- [x] **v0.2:** Consolidated commitments listed in [`project/v0.2-roadmap.md`](v0.2-roadmap.md) per Q-Frank-Step9-A Ask 6 (phasing out the soft "v0.2 ELK closes that" placeholder pattern). Headline entries: **Layer A consistency-affirmation gap (load-bearing entry; the gap Case B was pulled from `demo_p3.html` for); ELK reasoner integration (closes EL-profile gap); SROIQ integration (v0.3+); SLG tabling for SLD termination (replaces ADR-013 visited-ancestor cycle-guard); per-class Skolem-witness satisfiability checking refinement; OFI deontic ARC module per ADR-008 deferral; multi-consumer verification gate participation**.
