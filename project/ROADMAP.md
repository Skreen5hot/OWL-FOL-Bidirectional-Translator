...
**Current Phase:** Phase 1 — Built-In OWL Lifter (IN PROGRESS, Steps 1-7 of 9 complete; CI green locally on Step 7 close pending push; CI green on `c703ada` Step 6 close run `25275468060` and on `55111f2` corpus: BFO/CLIF parity fixture run `25276098017`). ADR-007 ratified Accepted at Step 5 close per architect Ruling 1; §7 (cardinality witness convention) resolved at Step 7 close; §8 (RDFC-1.0 b-node Skolem prefix) resolved at Step 6 close. **Phase 1 corpus Ring 1 closed at Step 7** — 14 fixtures + 3 inline regressions all active and passing. Phase 0 EXITED 2026-05-02.
...
- [~] `owlToFol()` per API spec §6.1 with structured OWL input handling per API spec §3 (Steps 1-7 complete; Steps 8-9 outstanding)
