...
### Phase 3 exit-packet ratification cycle (2026-05-10)

[Architect-banked at Phase 3 exit-packet ratification cycle 2026-05-10 (the brief follow-up cycle the Q-Frank-Step9-A corrective ruling sequencing anticipated). The architect verified the now-corrective-overlaid `project/reviews/phase-3-exit.md` against the four named verification points (pulled-Case-B framing; withdrawn-discipline framing; count-resolution framing; v0.2-roadmap.md authoring) plus four additional spot-check surfaces (cycle accounting consistency; honest-admission distinction across artifact categories; banking-correction discipline visibility; Frank's letter banking) — all eight correspondence checks passed. **Phase 3 exit packet RATIFIED. Phase 3 closes with this ratification.** The architect issued three new banked principles from this cycle observing the SME's exit-packet shape as exemplary practice.]

#### Phase exit packets containing banking corrections enumerate the §5 inventory as withdrawn + preserved + new + meta-banking categories with explicit counts and per-item attribution

The enumeration preserves the corrective character at the audit-trail layer. **Why:** without this accounting, withdrawals risk being absorbed silently into the new bankings; the cycle's corrective character would be invisible at the audit-trail layer. With this accounting, future readers see exactly what was withdrawn, what survived on its own merits, what was added new, and what meta-banking the cycle produced — corrective character preserved through enumeration. **How to apply:** at exit-packet authoring for any phase containing banking corrections, the §5 Banked Principles inventory uses the four-category framing: "N withdrawn + M preserved + K new + J meta-banking" with per-item attribution to the originating cycle. The Phase 3 exit packet's §5 inventory ("3 withdrawn + 2 preserved + 7 new + 1 meta-banking") is the canonical example. (§5 enumeration discipline)

#### Phase exit packets carrying banking corrections include a WITHDRAWN forward-tracks subsection at §6 explicitly listing forward-tracks that were created by prior rulings and withdrawn by corrective cycles

The subsection preserves inheritance-manifest accuracy across phase boundaries. **Why:** the same discipline that applies at the §5 banking surface (explicit withdrawal accounting) applies at the §6 forward-track surface. Forward-tracks created by prior rulings and withdrawn by corrective cycles get explicit "WITHDRAWN" framing rather than silent removal — otherwise next-phase entry-packet authoring could accidentally inherit withdrawn forward-tracks by reading prior phase exit packets that did include them. **How to apply:** at exit-packet authoring for any phase containing banking corrections, the §6 Forward-Tracks section gains a "WITHDRAWN forward-tracks" subsection listing items withdrawn by corrective cycles with the corrective ruling's reason cited. The Phase 3 exit packet's §6 WITHDRAWN subsection (count-divergence corpus-discriminator question + disposition-split discipline as Phase-4-entry methodology candidate) is the canonical example. (§6 WITHDRAWN subsection discipline)

#### When phase exit retro candidates are withdrawn by corrective cycles, the slot is repurposed for retro candidates that the corrective cycle itself produces

The repurposing preserves retro depth across corrective cycles. **Why:** without slot repurposing, corrective cycles that withdraw a retro candidate would shrink the retro's depth by one; the retro would lose methodological reflection capacity proportional to the corrections issued. Corrective cycles typically generate methodological data points worth retro reflection (e.g., does the corrective discipline generalize? when should withdrawals be preferred to refinements?), so the slot a withdrawn candidate occupied is naturally repurposed for the candidate the corrective cycle itself produced. **How to apply:** at exit-packet authoring for any phase where a retro candidate is withdrawn by a corrective cycle, name the replacement candidate explicitly with the corrective cycle's attribution. The Phase 3 retro candidate slot vacated by the disposition-split methodology candidate (withdrawn at Q-Frank-Step9-A Ask 5) was repurposed for the banking-correction discipline retro analysis (introduced by the Q-Frank-Step9-A meta-banking) — the canonical example. (Retro candidate replacement discipline)
...
Four methodology-refinement candidates for Phase 3 exit retro per the architect's authoritative list at the Phase 3 exit-packet ratification cycle 2026-05-10:
...
3. **Retroactive-ritual-pattern-confirmation** — the verification ritual operationalized at Step 4 paid first production catch at Step 5 (spec §3.4.4 reference error in routing artifact) + first batch dividend at the retroactive corrective cycle 2026-05-09 (4 hypothetical fixtures' FOL @type discriminator corrections). Methodology refinement question for retro: does the retroactive-ritual pattern (architect-ratified discipline → operationalization → first production catch → first batch dividend confirmation) generalize as a Phase 4+ practice? When should retroactive batch runs be scoped at phase boundaries vs mid-phase? Does the production-dividend banking pattern apply to disciplines beyond the verification ritual? *(Note: at-risk-tag-conservatism observation — 2/2 at-risk parity canaries survived at Step 2 reactivation; conservative tagging preserved publication contract integrity without manifesting as actual divergence — surfaces as a sub-observation under this candidate per the architect's exit-packet ratification list, rather than as a separate retro candidate.)*

4. **Banking-correction discipline retro analysis** (per Q-Frank-Step9-A 2026-05-10 + this exit-packet ratification cycle's three new bankings) — the architect issues banking corrections (withdrawals + new bankings) when stakeholder critique surfaces concerns that hold on the merits. Methodology refinement question for retro: (a) what is the threshold for "concerns that hold on the merits" — when does the architect revise vs defend? (b) what cadence should banking corrections follow when multiple prior bankings need revision (single corrective cycle vs sequential revision)? (c) does the banking-correction discipline generalize to non-stakeholder-routing cycles (e.g., when an architect notices a prior ruling was structurally incorrect during their own review)? (d) do the §5/§6/retro-replacement disciplines banked at this exit-packet ratification cycle generalize to Phase 4+ exit packets, or are they Phase 3-specific corrective-character preservation patterns? Frank's letter ([`demo/Phase3DemoCritique.md`](../demo/Phase3DemoCritique.md)) banked as canonical exemplar of the post-hoc-discipline-canonization risk pattern.

These four exit-retro candidates are documented in `project/reviews/phase-3-exit.md` deferred-with-structural-requirements bucket. The Q-Frank-Step9-A corrective ruling adds the following Phase 4 entry-packet inheritance: (a) Case C — Lossy round-trip — as Phase 4 exit deliverable (NOT forward-track) per Ask 4; (b) Phase 2 reactivation surface as Phase 4 demo opening per Ask 7 SME election; (c) purpose-built Layer A consistency-check parity fixture question per Ask 8; (d) Layer A consistency-affirmation gap closure path per the consolidated `project/v0.2-roadmap.md` (NOT a "disposition-split discipline" methodology candidate — that forward-track is withdrawn per Ask 5). The Phase 3 exit-packet ratification cycle's three new bankings (§5 enumeration / §6 WITHDRAWN subsection / retro candidate replacement disciplines) inherit at Phase 4 entry-packet authoring per the standard exit-packet → entry-packet inheritance pattern.
