...
3. Update `.github/workflows/pages.yml` — extend the staging step to copy any new fixture files the Phase N demo imports + any newly-vendored canonical sources under `arc/upstream-canonical/`. **If the Phase N demo exercises the composition layer (`createSession`, `loadOntology`, `evaluate`, `checkConsistency`):** the bundle transitively imports `tau-prolog` as a bare specifier AND tau-prolog ships as UMD with no ESM default export, so a three-pronged bridge is required (banked at Phase 3 Step 9.4-amendment-2 2026-05-09 per Aaron's routing dispatch; the 9.4-amendment's import-map-only pattern was insufficient because the bundle's `import pl from "tau-prolog"` is a default-import that fails on UMD's `module.exports = pl` shape with `SyntaxError: does not provide an export named 'default'`):
   - **(a) Sync `<script>` tags BEFORE the importmap block in the demo HTML's `<head>`:** `<script src="./tau-prolog/core.js"></script>` followed by `<script src="./tau-prolog/lists.js"></script>`. These run the UMD browser-direct-load branches, setting `globalThis.pl` and registering the lists Module on it.
   - **(b) Importmap routes the bare specifiers to bridge shims at `demo/tau-prolog-shims/`:** `"tau-prolog": "./tau-prolog/esm-shim.js"` and `"tau-prolog/modules/lists.js": "./tau-prolog/lists-esm-shim.js"`. The shims re-export `globalThis.pl` as default (for `tau-prolog`) and a no-op function (for `tau-prolog/modules/lists.js`, since the sync-load already registered the module).
   - **(c) Staging block extension in `pages.yml`:** `cp node_modules/tau-prolog/modules/core.js node_modules/tau-prolog/modules/lists.js demo/tau-prolog-shims/esm-shim.js demo/tau-prolog-shims/lists-esm-shim.js gh-pages-deploy/tau-prolog/`.

   **Sequencing is load-bearing:** the sync `<script>` tags MUST execute before the module-script context evaluates, otherwise the shims' `globalThis.pl` reference is undefined. First instance is `demo_p3.html` + `demo/tau-prolog-shims/{esm-shim,lists-esm-shim}.js` (Phase 3 is the first per-phase demo whose bundle pulls tau-prolog into the browser ESM context). Phase 4+ composition-layer-using demos inherit this bridge-shim pattern; the shim files at `demo/tau-prolog-shims/` are reusable across phases (do NOT need to be regenerated per demo).
4. Push. CI deploys.
